...
export default withAdmin(async (req, res, _session, sql) => {
  if (req.method === 'GET') {
    const rows = await sql`
      SELECT
        t.id, t.name, t.display_name, t.description, t.status, t.created_at, t.updated_at,
        COUNT(DISTINCT ct.id)::int as usage_count
      FROM public.templates t
      LEFT JOIN public.company_templates ct ON ct.template_id = t.id
      GROUP BY t.id, t.name, t.display_name, t.description, t.status, t.created_at, t.updated_at
  ...
  if (req.method === 'POST') {
    ...
    const [row] = await sql`
      INSERT INTO public.templates (id, name, display_name, description, status, created_at, updated_at)
      VALUES (${id}, ${name.trim()}, ${dispName}, ${desc}, 'draft', ${now}, ${now})
